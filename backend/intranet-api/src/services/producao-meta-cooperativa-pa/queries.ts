...
export function getSql(tema: Tema) {
  ...
  PERIODO_REF AS (
    SELECT
      ...
      CASE
        WHEN PR.DT_FIM_SEMANA IS NOT NULL
          THEN LEAST(LAST_DAY(TRUNC(PR.DT_FIM_SEMANA)), TRUNC(SYSDATE))
        ELSE TRUNC(SYSDATE)
      END AS DT_REF_SNAPSHOT
    FROM PARAMS PR
  ),
...
AA_BASE_RAW AS (
  SELECT
    ...
    CASE
      WHEN REGEXP_LIKE(TRIM(AA.DT_MOVIMENTO), '^\\d{2}/\\d{2}/\\d{4}$')
        THEN TO_DATE(TRIM(AA.DT_MOVIMENTO), 'DD/MM/YYYY')
      WHEN REGEXP_LIKE(TRIM(AA.DT_MOVIMENTO), '^\\d{2}/\\d{2}/\\d{4}\\s+\\d{2}:\\d{2}:\\d{2}$')
        THEN TO_DATE(TRIM(AA.DT_MOVIMENTO), 'DD/MM/YYYY HH24:MI:SS')
      WHEN REGEXP_LIKE(TRIM(AA.DT_MOVIMENTO), '^\\d{4}-\\d{2}-\\d{2}$')
        THEN TO_DATE(TRIM(AA.DT_MOVIMENTO), 'YYYY-MM-DD')
      WHEN REGEXP_LIKE(TRIM(AA.DT_MOVIMENTO), '^\\d{4}-\\d{2}-\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}$')
        THEN TO_DATE(TRIM(AA.DT_MOVIMENTO), 'YYYY-MM-DD HH24:MI:SS')
      ELSE TRUNC(AA.DT_MATRICULA)
 ...
 AA_MOV_REF AS (
  SELECT PR.DT_REF_SNAPSHOT AS DT_MOV_REF_ESCOLHIDA
  FROM PERIODO_REF PR
),
  ...
  `;
    case "emprestimo_bancoob":
      ...
      CCBD_RAW AS (
        SELECT
          NR_PA_LOCAL_NEGOCIO,
          NR_COOPERATIVA_LOCAL_NEGOCIO,
          TRUNC(CAST(DT_LIBERACAO_CREDITO AS DATE)) AS DT_MOV,
          NVL(VL_CONTRATO, 0) AS VL_CONTRATO_NUM,
          ROW_NUMBER() OVER (
            PARTITION BY COALESCE(
              NULLIF(TRIM(TO_CHAR(NR_CONTRATO_CREDITO)), ''),
              NULLIF(TRIM(TO_CHAR(NR_PROPOSTA)), ''),
              TRIM(TO_CHAR(NR_CPF_CNPJ_CLIENTE))
                || '|'
                || TO_CHAR(CAST(DT_LIBERACAO_CREDITO AS DATE), 'YYYYMMDD')
                || '|'
                || TO_CHAR(NVL(VL_CONTRATO, 0))
            )
            ORDER BY
              TRUNC(TO_DATE(DT_MOVIMENTO, 'DD/MM/YYYY')) ASC NULLS LAST,
              TRUNC(CAST(DT_LIBERACAO_CREDITO AS DATE)) ASC NULLS LAST,
              NVL(NR_PA_LOCAL_NEGOCIO, 999999)
          ) AS RN
    ...
    CDN_RAW AS (
      SELECT
        ...
        PARTITION BY
          ...
          ORDER BY
            CASE WHEN SN_VENDA_CONCLUIDA = 'SIM' THEN 0 ELSE 1 END,
            CASE WHEN SITUACAO_COTA <> 'EXCLUIDO' THEN 0 ELSE 1 END,
            CASE WHEN DESC_VERSAO_COTA = 'ATIVA' THEN 0 ELSE 1 END,
            TRUNC(TO_DATE(DT_ADESAO,'DD/MM/YYYY'))
        ) AS RN,
        MAX(
          CASE
            WHEN UPPER(TRIM(SITUACAO_COTA)) = 'EXCLUIDO' THEN 1
            ELSE 0
          END
        ) OVER (
          PARTITION BY
            NVL(TRIM(TO_CHAR(NR_PA)), ' '),
            NVL(TRIM(TO_CHAR(NR_COOPERATIVA)), ' '),
            TRUNC(TO_DATE(DT_ADESAO,'DD/MM/YYYY')),
            NVL(UPPER(TRIM(NM_VENDEDOR)), ' '),
            NVL(VL_CONTRATADO,0)
        ) AS TEM_EXCLUIDO
