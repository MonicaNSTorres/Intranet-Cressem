...
function sqlSeguroVendaNova() {
  return `
    ...
    SELECT
      BF.NM_FUNCIONARIO,
      BF.ANO,
      NVL(MJ.PRODUCAO_MES, 0) AS PRODUCAO_VIGENTE,
      NVL(MM.META_MES, 0) AS META_VIGENTE,
      NVL(MJ.PRODUCAO_MES, 0) - NVL(MM.META_MES, 0) AS GAP_VIGENTE,
      NVL(MJ.PRODUCAO_MES, 0) AS PRODUCAO_MENSAL,
      NVL(MM.META_MES, 0) AS META_MENSAL,
      CASE
        WHEN NVL(MM.META_MES, 0) > 0
    ...
    WITH
      ...
      PVPD_BASE_ALL AS (
        ...
        WHERE PVPD.DT_PROPOSTA IS NOT NULL
          ...
          AND TRIM(PVPD.DESC_PRODUTO) NOT IN (
            'SQ-NOVO PECÚLIO PREVI',
