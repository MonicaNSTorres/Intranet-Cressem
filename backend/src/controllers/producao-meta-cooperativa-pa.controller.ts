...
BASE_TABELAS AS (
  ...
  SELECT 'CARTAO_CREDITO_BANCOOB_DIARIO_NORMALIZADO' FROM DUAL UNION ALL
  ...
  SELECT 'CONTA_CAPITAL_DIARIO_NOVO_NORMALIZADO' FROM DUAL UNION ALL
  SELECT 'CONTA_CORRENTE_DIARIO_NOVO' FROM DUAL UNION ALL
  SELECT 'CONTA_CORRENTE_DIARIO_NOVO_NORMALIZADO' FROM DUAL UNION ALL
  ...
  SELECT 'SEGURO_VENDA_NOVA_ATUALIZADA' FROM DUAL UNION ALL
...
MOV AS (
  ...
  SELECT
    'CARTAO_CREDITO_BANCOOB_DIARIO_NORMALIZADO',
    MAX(
      CASE
        WHEN REGEXP_LIKE(TRIM(DT_MOVIMENTO), '^\\d{2}/\\d{2}/\\d{4}$')
          THEN TO_DATE(TRIM(DT_MOVIMENTO), 'DD/MM/YYYY')
  ...
  SELECT
    'CONTA_CAPITAL_DIARIO_NOVO_NORMALIZADO',
    MAX(
      CASE
        WHEN REGEXP_LIKE(TRIM(DT_MOVIMENTO), '^\\d{2}/\\d{2}/\\d{4}$')
          THEN TO_DATE(TRIM(DT_MOVIMENTO), 'DD/MM/YYYY')
  ...
  FROM DBACRESSEM.CONTA_CAPITAL_DIARIO_NOVO_NORMALIZADO
  ...
  SELECT
    'CONTA_CORRENTE_DIARIO_NOVO_NORMALIZADO',
    MAX(
      CASE
        WHEN REGEXP_LIKE(TRIM(DT_MOVIMENTO), '^\\d{2}/\\d{2}/\\d{4}$')
          THEN TO_DATE(TRIM(DT_MOVIMENTO), 'DD/MM/YYYY')
  ...
  FROM DBACRESSEM.CONTA_CORRENTE_DIARIO_NOVO_NORMALIZADO
  ...
  SELECT 'SEGURO_VENDA_NOVA_ATUALIZADA', MAX(TRUNC(DT_VENDA))
  FROM DBACRESSEM.SEGURO_VENDA_NOVA_ATUALIZADA
...
SELECT
  B.NM_TABELA,
  COALESCE(L.DT_CARGA, M.DT_MOVIMENTO) AS DT_CARGA,
  M.DT_MOVIMENTO
FROM BASE_TABELAS B
LEFT JOIN LOG L
  ON L.NM_TABELA = B.NM_TABELA
