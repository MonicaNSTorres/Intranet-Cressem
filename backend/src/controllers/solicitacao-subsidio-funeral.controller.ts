...
const SMB_BASE_FOLDER = "CRM/SUBSIDIO_FUNERAL";
...
async function salvarAnexoLinux(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "subsidio-funeral-smb-"));
...
async function readFileFromSmbLinux(caminhoOriginal: string) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "subsidio-funeral-smb-download-"));
    ...
    try {
      ...
      if (acao === "ENVIAR_DIRETORIA" || acao === "ENVIAR_FINANCEIRO" || acao === "REENVIAR") {
        if (!podeAtuarComoSolicitante) {
          return res.status(403).json({
            error: "Somente quem abriu a solicitação pode enviar ao financeiro.",
          });
        }
