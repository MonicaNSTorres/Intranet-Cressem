...
export function DemissaoForm() {
  ...
  const [idBeneficiarioConvenio, setIdBeneficiarioConvenio] = useState<number | null>(null);
  const [loadingGerar, setLoadingGerar] = useState(false);
  const [nomeAtendente, setNomeAtendente] = useState("Atendente");
  const [loginAtendente, setLoginAtendente] = useState("");
  ...
  useEffect(() => {
    async function carregarAtendente() {
      try {
        ...
        const login = String(me?.username || me?.email || "").trim();
        if (nome) setNomeAtendente(nome);
        if (login) setLoginAtendente(login);
      } catch {
        // Mantém fallback.
      }
    }
  ...
  function limparFormulario() {
    ...
    setValorConvenio("");
    setIdBeneficiarioConvenio(null);
    setErro("");
    setInfo("");
  }
  ...
  const onBuscar = async () => {
    try {
      ...
      if (convenio?.titular_ativo) {
        ...
        setIdBeneficiarioConvenio(Number(convenio.idBeneficiario) || null);
        setValorConvenio(
  ...
  const gerar = async () => {
    try {
      ...
      if (possuiConvenio === "Sim") {
        try {
          if (!idBeneficiarioConvenio) {
            throw new Error("Beneficiário do convênio não identificado. Consulte o associado novamente antes de gerar o PDF.");
          }
          const resultado = await desativarConvenioDemissao(idBeneficiarioConvenio, {
            nomeUsuario: nomeAtendente || "Atendente",
            loginUsuario: loginAtendente || nomeAtendente || "ATENDENTE",
            observacao: "Desligamento realizado via formulário de Demissão.",
            origem: "DEMISSAO",
          });
          emailConvenioEnviado = Boolean(resultado?.emailEnviado);
        } catch (error: any) {
          falhaConvenio =
        ...
        setInfo(
          possuiConvenio === "Sim"
            ? emailConvenioEnviado
              ? "PDF gerado, convênio odontológico desativado e e-mail enviado ao solicitante e ao Cadastro."
              : "PDF gerado e convênio odontológico desativado. Atenção: a notificação por e-mail não foi enviada."
