...
export async function gerarPdfTermoMensalCaixa(o: PdfOpts) {
  ...
  const formatDataBaseCompetencia = (value: string) => {
    if (!value?.includes("-")) return "__/__/____";

    const [ano, mes] = value.split("-");
    const data = new Date(Number(ano), Number(mes) - 1, 0);
  ...
  async function desenharPagina(tituloAgencia: string) {
    y = 48;
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7);
    doc.text(tituloAgencia, margin, y);
    y += 8;
    ...
    const dataFormatada = formatDataBaseCompetencia(o.competencia);
  ...
  await desenharPagina(`Agência ${toUpper(o.pa || "SEDE")}`);

  doc.addPage();

  await desenharPagina("ATM EXTERNO (Prosegur)");
